...
def login_required(f):
    ...
    def wrap(*args, **kwargs):
        try:
            ...
            session = json.loads(b64decode(cookie))
            for u in users:
               if u.username == session[USER] and u.passhash == session[SECRET]:
                   g.user = u
                   return f(*args, **kwargs)
...
def login_user(u, form):
    ...
    cookie = b64_cookie({USER: u.username, SECRET: u.passhash})
    ...
    if form.validate_on_submit():
        ...
        if user is None:
            ...
            if user.is_admin or user.username == 'admin':
                flash('The princess is in another castle (nice try!)')
                return redirect(url_for('.reset_1'))
            sendmail('Password Reset Request', '''
            Click <a href="%s?token=%s">here</a> to reset your password!
            ''' % (url_for('.reset_2', uname=user.username), sha1(user.username)))
...
def inbox():
    ...
    return render_template('inbox.html', title='LMail', subtitle='Your totally legit email inbox!', emails=emails)
