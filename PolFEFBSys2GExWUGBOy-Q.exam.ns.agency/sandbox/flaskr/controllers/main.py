...
def trash_post():
    ...
    if form.validate_on_submit():
        ...
        q = '''
        INSERT INTO trash (
          uuid, idx, views, highlight, is_public, timestamp, user_id,
          title, content, password
        ) VALUES ('%s', %d, %d, '%s', %s, '%s', %s, '%s','%s', '%s')
        ''' % (
            tid, default_trash_index(), 0, form.highlight.data, public, datetime.utcnow(), uid,
            form.title.data, sql_4(form.content.data), form.password.data,
        )
        try:
            db.engine.execute(q)
...
def trash_page(id):
    ...
    if t is None:
        flash("Nice try, but you should really inserting new trash instead!")
        return redirect(url_for('.index'))
