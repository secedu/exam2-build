...
def trash_page(id):
    ...
    t = Trash.query.filter_by(uuid=str(id)).first()


    if t is None:
        flash("Nice try, but you should really check out our password protection on trashes. We *BLIND* people to your secrets!")
        return redirect(url_for('.index'))
    ...
    try:
        valid = False
        result = db.engine.execute("SELECT password from trash where uuid='%s' AND password='%s'" % (tid, password))
        for row in result:
            check_pass = row[0]
            valid = True
        if not valid:
            return render_template('base.html', title='Internal Error', subtitle='User/Password Not Found')
    except Exception as e:
        return render_template('base.html', title='Internal Error', subtitle='Invalid SQL query.'), 500
