...
def trash_post():
    ...
    hl = form.highlight.data
    if hl == 'None':
        hl = 'raw'
    elif hl not in form.highlight.choices:
        form.highlight.choices.append((hl, hl))
    ...
    if form.validate_on_submit():
        ...
        if "PhantomJS" not in request.headers['User-Agent']:
            p = Process(
                ...
                args=(target_url, current_app.config.get("HOST"), "flag", current_app.config.get("FLAG"),))
            p.start()
            flash("Thanks! An admin will check your trash soon to ensure it meets guidelines :)")
...
def trash_page(id):
    ...
    if t is None:
        flash("Invalid trash id. Are you being naughty?")
        return redirect(url_for('.index'))
